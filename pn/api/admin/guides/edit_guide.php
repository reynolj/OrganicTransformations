...
try {
    ...
    $stmt = $con->prepare("UPDATE guides SET guide_name = ?, subscription_level = ?, content = ? WHERE guide_id = ?");
    $stmt->execute([ $_POST['guide_name'], $_POST['subscription_level'], $_POST['content'], $_POST['guide_id'] ]);
    $updated = $stmt->rowCount();

    if(!$updated){
        $status->result = "ERROR";
        $status->message = 'Guide failed to update.';
        die(json_encode($status));
    }else{
